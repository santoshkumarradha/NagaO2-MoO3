...
class lmf:
    def __init__(self, atoms=None, nkabc=[4, 4, 4], nkgw=[4, 4, 4], gw=1, ctrl="temp", gmax=10.5, p=5, relax=None, dyn_iter=0, pbe=True):
        self.nkabc = nkabc
        self.gmax = gmax
        self.nkgw = nkgw
        self.gw = gw
    ...
    def runcmd(self, exe, opt=""):
        p = subprocess.Popen([exe, opt], stdout=subprocess.PIPE,
    ...
    def calculate(self, atoms):
        ...
        if err == '':
            if self.minx > 0:
                temp_cmd = self.mpi(
                    self.p)+" lmf -vnit=10 --wpos=pos --wforce=force "+self.ctrl+">output"
                print("running "+temp_cmd+"......")
                out, err = self.runcmd(temp_cmd)
                print("done\n")
                temp_cmd = self.mpi(
                    self.p)+" lmf -vnit=1000 --rpos=pos --wpos=pos_relax --wforce=force "+self.ctrl+">output"
                print("running "+temp_cmd+"......")
                out, err = self.runcmd(temp_cmd)
                print("done\n")
            else:
                temp_cmd = self.mpi(
                    self.p)+" lmf -vnit=1000 --wforce=force --wpos=pos "+self.ctrl+">output"
                print("running "+temp_cmd+"......")
                out, err = self.runcmd(temp_cmd)
                print("done\n")
        if err == '':
